...
function Quiz() {
  ...
  const renderQuestion = () => {
    ...
    if (!categoryQuestions || currentQuestion >= categories.length * categoryQuestions.length) {
      return renderResults();
    }
  ...
  const renderResults = () => {
    const categoryScores = Object.keys(questions).reduce((scores, category) => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        if (response && questions[category][i % questions[category].length].options.includes(response)) {
          scores[category]++;
        }
      }
